...
std::string path {"D:/Coding/C++ - UDEMY/P20 - The Standard Template Library (STL)/17. Challenge 3"};
...
void part1() {
    ...
    std::string line;       
    std::string word;   
    std::ifstream in_file {path + "/words.txt"};
    if (in_file) {
...
void part2() {
    ...
    if (in_file) {
        ...
        while (std::getline(in_file, line)) {
            ...
            while (ss >> word) {
                word = clean_string(word);
                words[word].insert(line_numbers);
            }
        }
